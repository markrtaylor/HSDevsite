!function(e){
	function t(i,o) { 
		...
		this._handlers = { 
			...
			e.each(t.Plugins, e.proxy(function(e,t) { this._plugins[e[0].toLowerCase() + e.slice(1)]=new t(this)},this))
			this.initialize()}
	...
	var i;i={MF000:"Sent",
			...
			MF005:"Email capture text file error!"
	...
	t.Plugins={}
	t.prototype.initialize=function(){ this.$element.trigger("mf.initialize")
	...
	t.prototype.watch = function(){var e=this;e.$element.ajaxForm({ beforeSubmit:function() { e.$element.trigger("mf.process")}
						error:function(t){ e.$element.trigger("mf.fail"
							{code:t,message:i[t]})}
						success:function(t){ console.log(t),
							"MF000"==t?e.$element.trigger("mf.success",
								{code:t,message:i[t]}):(t=5==t.length?t:"MF255"
								e.$element.trigger("mf.fail",
								{code:t,message:i[t]}))}}).on(this._handlers)}

	t.prototype.process = function(){this.$element.addClass("process")}
	t.prototype.update = function(t,i){this.$element.removeClass("process")
		this.$element.addClass("MF000"===i.code?"success":"fail")
		setTimeout(e.proxy(function(){this.$element.trigger("reset")},this),3e3)}

	t.prototype.reset = function(){this.$element.removeClass("success")
						this.$element.removeClass("fail")
						this.$element.trigger("mf.reset")}

	e.fn.rdMailForm=function(i){ return this.each(function() {
							e(this).data("rdMailForm")||e(this).data("rdMailForm",new t(this,i))})}

	e.fn.rdMailForm.Constructor=t }
	(window.jQuery,window,document)
	function(e){
		var t = e.fn.rdMailForm.Constructor.Plugins.Validator = function(i){this._core=i,this._handlers={"mfValidator.validate":this.validate,"mfValidator.error":this.error,"mfValidator.valid":this.valid,"mfValidator.reset":this.reset,"mfValidator.click":e.noop()},this._core.options=e.extend({},t.Defaults,this._core.options),this.initialize()};t.Defaults={validator:{applyTo:"[data-constraints]","class":"mfValidation",constraints:{"@LettersOnly":{rule:"^([a-zA-Zа-яА-ЯіїёІЇЁєЄҐґ\\s]{0,})$",message:"Please use letters only!"},"@NumbersOnly":{rule:"^-?\\d*\\.?\\d*$",message:"Please use numbers only!"},"@NotEmpty":{rule:"([^\\s])",message:"Field should not be empty!"},
		"@Email":{rule:"^(([\\w-]+(?:\\.[\\w-]+)*)@((?:[\\w-]+\\.)*\\w[\\w-]{0,66})\\.([a-z]{2,6}(?:\\.[a-z]{2})?)){0,}$",
		message:"Enter valid e-mail address!"},
		"@Phone":{rule:"^(\\+?\\d{0,3}\\s*\\(?\\d{1,3}\\)?\\s*\\d{3}\\s*\\d{4}){0,}$"
		message:"Enter valid phone number!"},
		"@Date":{rule:function(e){return navigator.userAgent.match(/(iPod|iPhone|iPad)/)?!0:new RegExp("^($)|(((0[13578]|10|12)(-|\\/)((0[1-9])|([12])([0-9])|(3[01]?))(-|\\/)((19)([2-9])(\\d{1})|(20)([01])(\\d{1})|([8901])(\\d{1}))|(0?[2469]|11)(-|/)(([1-9])|(0[1-9])|([12])([0-9]?)|(3[0]?))(-|/)((19)([2-9])(\\d{1})|(20)([01])(\\d{1})|([8901])(\\d{1}))))$").test(e.val())}
		,message:"Use MM/DD/YYYY format!"},
		"@SelectRequired":{rule:function(e){return 0!==e.find("option:selected").index()}
